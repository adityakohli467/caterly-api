...
export class PricingService {
  ...
  calculateProductPrice(
    ...
    if (wholesalePrice !== null && wholesalePrice !== undefined) {
      ...
      const discount = parseFloat((retailDiscountPercentage || 40).toString());
      calculatedWholesalePrice = originalRetailPrice * (1 - discount / 100);
    }

    // Determine base price based on customer type
    const basePrice = isWholesale ? calculatedWholesalePrice : originalRetailPrice;
