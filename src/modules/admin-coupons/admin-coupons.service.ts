...
export class AdminCouponsService {
  ...
  async create(createCouponDto: any): Promise<any> {
    ...
    const { coupon_code, coupon_description, coupon_discount, type, status } = createCouponDto;
    ...
    try {
      const result = await this.dataSource.query(
        `INSERT INTO coupon (coupon_code, coupon_description, coupon_discount, type, status)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING *`,
        [coupon_code, coupon_description, coupon_discount, type || 'F', status !== undefined ? status : 1],
      );
  ...
  async update(id: number, updateCouponDto: any): Promise<any> {
    const { coupon_code, coupon_description, coupon_discount, type, status } = updateCouponDto;
    ...
    const result = await this.dataSource.query(
      ...
      WHERE coupon_id = $6
      RETURNING *`,
      [coupon_code, coupon_description, coupon_discount, type, status, id],
    );
