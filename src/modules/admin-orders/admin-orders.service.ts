  ...
  async findAll(query: any): Promise<any> {
    ...
    const orders = result.map((row: any) => {
      ...
      return {
        ...
        location_abn: row.location_abn,
        location_email: row.location_email,
        location_address: row.location_address,
        location_phone: row.location_phone,
        location_id: row.location_id,
        delivery_date: row.delivery_date_time ? new Date(row.delivery_date_time).toLocaleDateString('en-CA', { timeZone: 'Australia/Sydney' }) : null,
        delivery_time: row.delivery_date_time ? new Date(row.delivery_date_time).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false, timeZone: 'Australia/Sydney' }) : null,
