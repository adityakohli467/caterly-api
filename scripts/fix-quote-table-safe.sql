--
-- Quote Table Setup (Safe - handles existing objects)
-- This file safely creates the quote table even if parts already exist
--
...
DROP SEQUENCE IF EXISTS public.quote_order_id_seq CASCADE;
...
CREATE TABLE public.quote (
    order_id integer NOT NULL DEFAULT nextval('public.quote_order_id_seq'::regclass),
    ...
    CONSTRAINT quote_pkey PRIMARY KEY (order_id)
);

-- Create indexes
CREATE INDEX IF NOT EXISTS idx_quote_customer ON public.quote(customer_id);
CREATE INDEX IF NOT EXISTS idx_quote_status ON public.quote(order_status);

-- Add foreign key constraints (only if referenced tables exist)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'customer') THEN
        ALTER TABLE public.quote ADD CONSTRAINT quote_customer_id_fkey 
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'locations') THEN
        ALTER TABLE public.quote ADD CONSTRAINT quote_location_id_fkey 
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'user') THEN
        ALTER TABLE public.quote ADD CONSTRAINT quote_user_id_fkey 
